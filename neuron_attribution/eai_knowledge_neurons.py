...
class KnowledgeNeurons:
    ...
    def _prepare_inputs(self, prompt, target=None, encoded_input=None):
        ...
        if self.args.dataset == "csqa":
            ...
            if self.args.dataset == "hans":
                claim = self.tokenizer.tokenize("Premise : %s" %self.examp_obj.claim)
                evidence = self.tokenizer.tokenize("/ Hypothesis : %s" %self.examp_obj.evidence)
                answer = self.tokenizer.tokenize("/ Answer :")

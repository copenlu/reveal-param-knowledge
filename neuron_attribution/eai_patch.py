...
class Patch(torch.nn.Module):
    ...
    def __init__(
        ...
        assert mode in ["replace", "suppress", "enhance", "sufficiency"]
        self.mode = mode
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.mode == "replace":
            # csqa
            # if self.end_pos != []:
            #     self.acts = self.acts.squeeze(1)
            #
            #     end_pos = torch.tensor(self.end_pos)
            #     if len(x.shape) == 2:
            #         x[end_pos, :] = self.acts
            #     else:
            #         x[:, end_pos, :] = self.acts
        ...
        elif self.mode == "sufficiency":
            # pdb.set_trace()
            # new_x = torch.zeros(x.shape).to("cuda")
            new_x = torch.clone(x)
            ...
            if self.end_pos != []:
                for mask_id in self.end_pos:
                    ...
                    for pos in self.target_positions:
                        if len(x.shape) == 2:
                            new_x[mask_id, pos] = x[mask_id, pos]
                        else:
                            # print(x[:, self.mask_idx, pos])
                            new_x[:, mask_id, pos] = x[:, mask_id, pos]
                ...
                for pos in self.target_positions:
                    if len(x.shape) == 2:
                        new_x[self.mask_idx, pos] = x[self.mask_idx, pos]
                    else:
                        # print(x[:, self.mask_idx, pos])
                        new_x[:, self.mask_idx, pos] = x[:, self.mask_idx, pos]

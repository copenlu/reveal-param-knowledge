from eai_knowledge_neurons import *
...
def load_examples(args, mode="train"):
  if args.dataset in ["fever", "snli", "csqa", "mnli", "hans"]:
    filepath = mode
  ...
  if args.dataset == "hans":
    examples = CREATE_FUNCTIONS[args.dataset](filepath, mode, model_name=args.model_type.split("/")[-1])
  else:
...
def run_neuron_attr(examples, label_dict, model, tokenizer, args):
  ...
  for idx, examp in tqdm(enumerate(example_sample_list)):
    ...
    # pdb.set_trace()
    if args.dataset in ["snli", "mnli", "hans"]:
    ...
    # prompt ver1
    prompt = "Claim : %s/ Evidence : %s/ Answer :" %(examp.claim, examp.evidence)

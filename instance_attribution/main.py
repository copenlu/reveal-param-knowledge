...
from transformers import AutoTokenizer, OPTForSequenceClassification, GPTNeoXForCausalLM, GPTNeoForCausalLM, BloomForSequenceClassification
from transformers import GPTNeoXForSequenceClassification

from utils_influence import *
...
def load_dataset(tokenizer, args, mode="train"):
    # for hans - it should be mnli train dataset
...
def run_similarity_funcs(train_dataset, model, args):
    ...
    for idx, sample in tqdm(enumerate(train_loader_sim_func)):
        ...
        # batch_grads = autograd.grad(outputs[0], pooled_output["0"])[0]
        # last_idx = torch.nonzero(batch_grads[0,:,0]!=0.0).item()
        # batch_grads = batch_grads[:, last_idx, :].squeeze(0)
        # batch_grads.shape : (768)

        # pred_logits, pred_max = torch.max(outputs[1], dim=1)
        # batch_grads = autograd.grad(pred_logits, target_params)[0].data.view(-1)

        # nn.C
        loss_for_batch = loss_f(outputs[1], batch[2])

        # when train batch size is bigger than 1
        batch_grads = []
        for lb in loss_for_batch:
            tmp_grads = autograd.grad(lb, target_params, retain_graph=True)[0].data.view(-1)
            batch_grads.append(tmp_grads)

        # when train batch size is 1
        # batch_grads = autograd.grad(outputs[0], target_params)[0].data.view(-1)
        # batch_grads.shape : (3072)

        train_grads += batch_grads
...
def get_only_ihvp(idx, batch, model, param_influence, train_dataset, args):
    model.zero_grad()
    test_loss = model(input_ids=batch[0], attention_mask=batch[1], labels=batch[2])[0]
    test_grads = autograd.grad(test_loss, param_influence)
...
def main(args):
    ...
    for idx, sample in enumerate(dev_dataloader):
        ...
        if args.IA_method == "inf-func":
            # get test prediction
            with torch.no_grad():
                result = model(input_ids=batch[0], attention_mask=batch[1], labels=batch[2])[1]
                pred_label = torch.argmax(result)

            # original
            # inf_score_list = do_inf_function(idx, batch, model, param_influence, train_dataset, train_dataloader, args)

            tmp_ihvp = get_only_ihvp(idx, batch, model, param_influence, train_dataset, args)
            ...
            # test_grads = autograd.grad(outputs[0], pooled_output["0"])[0]
            # last_idx = torch.nonzero(test_grads[0, :, 0] != 0.0).item()
            # test_grads = test_grads[:, last_idx, :].squeeze(0)

            # pred_logits, pred_max = torch.max(outputs[1], dim=1)
            # test_grads = autograd.grad(pred_logits, param_influence)[0].data.view(-1)
            test_grads = autograd.grad(outputs[0], param_influence)[0].view(-1)
